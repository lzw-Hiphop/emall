    ...
    <div class="navbar">
        <div class="navbar-inner">
            ...
            <form action="${pageContext.request.contextPath}/search" method="post" class="navbar-form pull-right">
                <input type="text" name="searchProduct" value="${searchProduct}" class="span2">
...
<div class="container">
    <div class="jumbotron">
        ...
        <hr>
        <div class="row">
            <c:forEach var="p" items="${pageUtil.list}">
                <div class="span3">
        ...
        <div class="pagination">
            <h5>共搜到${pageUtil.pageNumber}个结果</h5>
            <ul>
                <!--第一页-->
                <li>
                    <c:choose>
                    <c:when test="${pageUtil.pageIndex - 1 > 0}">
                    <a href="change_page?pageIndex=1&searchProduct=${searchProduct}" aria-label="Previous">
                ...
                <c:forEach var = "i" begin="1" end="${pageUtil.pageCount}" step="1">
                    <c:choose>
                        <c:when test="${pageUtil.pageIndex==i}">
                            <li class="active">
                                <a href="change_page?pageIndex=${i}&searchProduct=${searchProduct}">${i}</a>
                            </li>
                        </c:when>
                        <c:otherwise>
                            <li><a href="change_page?pageIndex=${i}&searchProduct=${searchProduct}">${i}</a></li>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
                <!--尾页-->
                <li>
                    ...
                    <a href="change_page?pageIndex=${pageUtil.pageCount}&searchProduct=${searchProduct}" aria-label="Next">
